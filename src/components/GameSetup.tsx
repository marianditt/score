...
export function GameSetup({ onStart, onCancel }: GameSetupProps) {
  ...
  const thresholdValid = typeof threshold === 'number' && threshold > 0;
  ...
  function getStartHint(): string {
    if (!gameName.trim()) return t('enterGameName');
    if (players.length === 0) return t('addPlayerFirst');
    if (!thresholdValid) return t('enterThreshold');
    return '';
  }
      ...
      <div className="max-w-lg mx-auto">
        ...
        <form onSubmit={handleStart} noValidate>
          ...
          <section className="bg-gray-800 rounded-xl border border-gray-700 p-4 mb-4" aria-labelledby="section-players">
            ...
            {players.length > 0 && (
              <ul className="space-y-2 mb-3" role="list" aria-label={t('playersSection')}>
                {players.map((player, idx) => (
                  ...
                  >
                    {/* Reorder buttons */}
                    <div className="flex flex-col gap-0.5" aria-label={player.name}>
                      <button
                        type="button"
            ...
            <div className="flex flex-wrap gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-32 bg-gray-700 border text-white rounded-lg px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent placeholder-gray-500 ${
                    threshold !== '' && !thresholdValid
                      ? 'border-red-500'
                      : 'border-gray-600'
                  }`}
                  aria-label={t('scoreThreshold')}
                />
                <p id="threshold-hint" className={`mt-1 text-xs ${threshold !== '' && !thresholdValid ? 'text-red-400' : 'text-gray-500'}`}>
                  {t('thresholdHint')}
                </p>
              </div>
            </div>
